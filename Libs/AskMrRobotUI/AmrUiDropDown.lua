...
local methods = {
	...
	["CreateItem"] = function(self, index)
		...
		item:SetScript("OnClick", function(frame, ...)
	...
	["RenderItems"] = function(self)
		...
		for i, obj in ipairs(self.itemlist) do
			...
			item:Show()
			item:SetNormalFontObject(
				Amr.CreateFont(
					obj.selected and "Bold" or "Regular", 
					obj.selected and self.fontHeight + 2 or self.fontHeight, 
					obj.color or Amr.Colors.White, 1
				)
			)
